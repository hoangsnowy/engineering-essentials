...
- **Database columns.** Every row pays the column's width, on the table **and in every index that includes it**, **and** in every backup, replica, and page cached in the buffer pool. Narrow a column from `int` (4 B) to `smallint` (2 B) across **one billion rows** and you save ~2 GB at rest — then again on each replica and backup. Narrower rows also mean **more rows per page**, so the same query reads fewer pages from disk. The saving is multiplied by your entire data footprint.
- **The wire.** An API that returns a million records pays for every needless byte in latency and egress bandwidth — which on a metered cloud is a literal line item. Binary protocols make this sharper: a protobuf `varint` encodes a small number in one byte and a large one in five, so the *value* and *type* directly set the payload size.
...
- A lone `short` field inside a class buys little. The CLR aligns fields and adds object overhead (header + padding), so one `short` among `int`s and references is typically padded back up to a word boundary. You spent clarity and gained no RAM.
...
- **`tinyint` is unsigned `0–255`** in SQL Server and maps cleanly to C# `byte`. There is no 1-byte *signed* SQL type, and C# `sbyte`/`uint`/`ulong` are not CLS-compliant — so for public APIs prefer the signed CLS types and pick width by range.
...
- **Surrogate keys: think about the ceiling.** `int` tops out near 2.15 billion. For `Booking.Id` in a busy global product that can be a real horizon; `bigint` is the safe default for high-volume tables, and the 4 extra bytes per row are justified there precisely because they *aren't* justified everywhere.
...
`float` and `double` are **binary** floating point: fast, enormous range, and **approximate** for most decimal fractions. `decimal` is **base-10** floating point: it represents decimal fractions *exactly*, at the cost of speed and range. They are tools for different jobs, not interchangeable sizes of the same tool.
...
| Percentage / ratio you display | `decimal` | `decimal(5,4)` | exact when shown/compared to humans |
